...
class Encoder(nn.Module):
    def __init__(self, x_dim, y_dim, z_dim, batch_size, seq_length=1):
        ...
        self.net_2 = nn.Linear(self.seq_length*self.z_dim, self.seq_length*self.z_dim*2)

    def encode(self, x, y):
        x = x.view(self.seq_length, self.batch_size, self.x_dim)
        y = y.unsqueeze(0).repeat(seq_length, 1, 1)
        ...
        for i in range(self.seq_length):
            z_m[i], h = torch.split(z, z.size(-1) // 2, dim=-1)
